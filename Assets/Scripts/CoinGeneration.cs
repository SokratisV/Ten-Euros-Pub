...
namespace Pub
{
    [CreateAssetMenu(fileName = "Coin Generation", menuName = "Pub/Coin Generation")]
    public class CoinGeneration : CoinGenerationBase
    {
        ...
        public override float[] GenerateCoins(System.Random rng)
        {
            ...
            foreach (var coinValue in CoinValues.Reverse())
            {
                ...
                {
                    if (IsAmountOfCoinsCorrect(amountOfCoins + 1) == false) //max value 20
                        return null;

                    if (totalValue + coinValue > gameData.TotalCoinValue) //max value 10
                        break;
